...
//--// Methods - Getters and Setters //--//		
    ...
    public void setHealth(double inStamina) {
    ...
    public double calcExp(ArrayList<Activity> heroActivities, Activity act) {
    	...
    	if (act instanceof Run) {
    		...
    		//increase the speed and strength by mph, health by level divided by number of hours
    		this.health = this.health + (this.level * (act.getDuration()/60) * (1+(act.getHR()/200)));
    		this.speed = this.speed + ((act.getDuration()/((Run) act).getDistance())/30);
    		this.strength = this.strength + (1+ (act.getDuration()/60))*((act.getHR()/200)+1);
            return runExp;
        } else if (act instanceof Bike) {
        	this.health = this.health + (this.level / (act.getDuration()/60));
    		this.speed = this.speed + ((act.getDuration()/((Bike) act).getDistance())/60);
    		this.strength = this.strength + (1+ (act.getDuration()/60/2))*((act.getHR()/200)+1);
        	double bikeExp =  3 * ((Bike) act).getDistance();
        	return bikeExp;
        } else if (act instanceof Swim) {
        	this.health = this.health + (this.level / (act.getDuration()/60));
    		this.speed = this.speed + ((act.getDuration()/((Swim) act).getDistance()));
    		this.strength = this.strength + (1+ (act.getDuration()/60/2))*((act.getHR()/200)+1);
        	double swimExp = base + 8 * ((Swim) act).getDistance();
            return swimExp;
        } else if (act instanceof Weights) {
        	this.strength = this.strength + this.level * ((Weights) act).getTotalVol()/100;
        	this.health = this.health + this.level * (act.getDuration()/60);
    ...
    public void expToNext() {
    	...
    	if (exp >= expNeeded) {
    		...
    		*/
    		System.out.println(this.getName() + " needs "+expDiff+" more experience points to reach level "+ nextLevel +" ... can we get there by the end of the month?");
    	}
    }
    
    /**
     * Method: advanceFloor()
     * Function: raises the hero floor level by 1
     * 
     */
    ...
    /**
     * Method: dropFloor()
     * Function: drops the hero floor level by 1 unless they're at 0
     * 
     */
    ...
    public void printHero() {
		...
		double heroHealth = this.getHealth();
		double heroStrength = this.getStrength();
		double heroSpeed = this.getSpeed();
