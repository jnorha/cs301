...
public class Main {
	public static void main(String args[]) {
		...
		//writes activity file with header if there was not one. 
		writeActivityCSV(activityFileName,activities2);
...
//--// Menu Methods //--//	
	...
	public static void mainMenu(ArrayList<Activity> activities, ArrayList<Hero> heroes, Scanner input) {
			...
			*/
			...
			String mainPrompt = " Your choice:";
			
			
			int choice = checkIntInput(input, mainPrompt);
			//dummy next line
			//input.nextLine();
			String heroFileName = "heroes.csv";
...
//--// Gameplay Methods
	...
	public static void floorChallenge(Hero hero, Scanner input) {
        ...
        double oppHealth = (oppLevel * 10) + rnd.nextInt(2,11);      // base 10×level plus 0–10 - ensures that their health won't start at 0
        double oppStrength = oppLevel * 2 + rnd.nextInt(5);
        double oppSpeed = (oppLevel * 2) + rnd.nextInt(10);// base 2×level plus 0–4 - opponent strength could be 0
        ...
        while (heroHealth > 0 && enemyHealth > 0) {
        	...
        	if (hero.getSpeed() >= opponent.getSpeed()) {
        		// Hero attacks
        		double heroDamage = ((hero.getStrength()+2) / 3) * rnd.nextInt(1,6);
                ...
                // Opponent attacks
                double oppDamage = ((opponent.getStrength()+2) / 3) * rnd.nextInt(1,6);
...
//--// Methods for writing to existing files/overwriting files
	...
	public static void writeActivityCSV(String fileName, ArrayList<Activity> activities) {
        ...
        try (BufferedWriter fw = Files.newBufferedWriter(filePath,StandardOpenOption.CREATE, StandardOpenOption.TRUNCATE_EXISTING)) {
            ...
            //System.out.println("Wrote " + activities.size() + " activities to " + filePath.toString());
        }
        catch (IOException ioe) {
            System.err.println("Failed writing CSV: " + ioe.getMessage());
        }
	...
	private static ArrayList<Activity> uploadActivity(Scanner input, ArrayList<Activity> activities, Hero hero) {
        ...
        try {
            ...
            */
            ...
            double duration = checkIntInput(input,durationPrompt);
...
//--//Methods for Reading Files //---------//
	...
	public static ArrayList<Activity> readActivityFile(String fileName, ArrayList<Hero> heroes) {
		...
		try {
			...
			//skip header if the first part of the string matches
			if(inputStr.split(",")[0].equals("hero")) {
