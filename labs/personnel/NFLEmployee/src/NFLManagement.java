/**
 * Course: CS300 - Summer 2025
 * Program: NFLManagement.java for Lab 1
 * 		gathers all the necessary information for an employee and generates the new object
 * Name: Josh Daniels
 * Wisc Email: jndaniels@wisc.edu
 * Date:	05/28/2025
 * Version:	1.0
 * @see NFLEmployee class 
 * 
 */

import java.util.*;
import java.io.*;

public class NFLManagement {
	public static void main(String args[]) {
		/*
		//Ask user to input the employee's name, team, job category, job title, salary, and contract end year 
		//(1) Create a scanner object to take input
		Scanner myScanner = new Scanner(System.in);
		//(2) Define variable to take the input
		System.out.println("What is the employee's name?");
		String name = myScanner.nextLine();
		System.out.println("What team is the employee on?");
		String team = myScanner.nextLine();
		System.out.println("What is the employee's job category?");
		String jobCategory = myScanner.nextLine();
		System.out.println("What is the employee's job title?");
		String title = myScanner.nextLine();
		System.out.println("What is the employee's salary?");
		int salary = myScanner.nextInt();
		System.out.println("What year does the employee's contract end?");
		int endYear = myScanner.nextInt();
		*/
		
		//
		
		//Create an employee object using the input values and the set constructor
		//NFLEmployee newEmployee = new NFLEmployee(name, team,jobCategory, title, salary, endYear);
	
		
//--//Create a menu for personnel management
		//personnelManagement(newEmployee);
		String personnelFileName = "C:\\sourcecode\\uwcs\\cs301\\labs\\personnel\\NFLPersonnel.csv";
		personnelManagement(personnelFileName);
		
	}
	
		
		
		// check the contract status for this new employee
		
	
	// need object as a parameter
	
	/**
	 * Method: managing personnel, including paystub and contract management
	 * @param employee
	 * @return none
	 */
	/**
	public static void personnelManagement(NFLEmployee newEmployee) {
		//Print out a menu, and ask the user to make the choice 
		System.out.println(" ============= Personnel Management Menu =================== ");
		System.out.println(" 1 - Managing the contract");
		System.out.println(" 2 - Managing the paystub");
		System.out.println(" Others - Exit");
		System.out.println(" Your choice:");
		Scanner input = new Scanner(System.in);
		int choice = input.nextInt();
		
		//Do the detailed management operations based on the choice
		// switch case here instead of if else
		switch(choice) {
		case 1: 
			manageContract(newEmployee);
			// break is important or it will run every case
			printContract(newEmployee);
			break;
			
		case 2:
			managePayStub(newEmployee);
			break;
			
		default: 
			System.out.println("Invalid choice, exiting ...");
			// could use break here or return
			return;
			
		}

	}
	*/
	/**
	 * Method: managing personnel, including paystub and contract managment
	 * @param personnel file name
	 * @return none
	 */
	
	public static void personnelManagement(String fileName) {
		//Print out a menu, and ask the user to make the choice 
		System.out.println(" ============= Personnel Management Menu =================== ");
		System.out.println(" 1 - Managing the contract");
		System.out.println(" 2 - Managing the paystub");
		System.out.println(" Others - Exit");
		System.out.println(" Your choice:");
		Scanner input = new Scanner(System.in);
		int choice = input.nextInt();
		
		//Do the detailed management operations based on the choice
		// switch case here instead of if else
		switch(choice) {
		case 1: 
			//manageContract(fileName);
			// break is important or it will run every case
			//printContract(fileName);
			System.out.println("To be done");
			break;
			
		case 2:
			managePayStub(fileName);
			break;
			
		default: 
			System.out.println("Invalid choice, exiting ...");
			// could use break here or return
			return;
			
		}

	}

	/**
	 * Method: Managing the contract for teh specific employee
	 * @param newEmployee
	 */
	
	/*
	public static void manageContract(NFLEmployee newEmployee) {
		
		if(newEmployee.getEndYear() <= 2024) {
			System.out.println("This employee's contract has expired. Their contract record should be remvoed from the database.");
			System.out.println("Expired Employee Contract Info:");
			System.out.println("Name="+newEmployee.getName()+", Team="+newEmployee.getTeam()+", Job Category="+newEmployee.getJobCategory()+", Title="+newEmployee.getTitle()+", Salary="+newEmployee.getSalary()+", Contract End Year="+newEmployee.getEndYear());
		}
		else if(newEmployee.getEndYear() == 2025) {
			Scanner newScanner = new Scanner(System.in);
			System.out.println("This employee's contract needs to be updated. Please provide the updated information: ");
			System.out.println("What is their updated job title? ");
			String newJobTitle = newScanner.nextLine();
			System.out.println("What is their updated annual salary?");
			int newSalary = newScanner.nextInt();
			System.out.println("What year does their new contract end?");
			int newEndYear = newScanner.nextInt();
			
			newEmployee.newContract(newJobTitle, newSalary, newEndYear);
			System.out.println("Updated Employee Contract Info:");
			System.out.println("Name="+newEmployee.getName()+", Team="+newEmployee.getTeam()+", Job Category="+newEmployee.getJobCategory()+", Title="+newEmployee.getTitle()+", Salary="+newEmployee.getSalary()+", Contract End Year="+newEmployee.getEndYear());
		}
		else {
			System.out.println("Employee Contract Info:");
			System.out.println("Name="+newEmployee.getName()+", Team="+newEmployee.getTeam()+", Job Category="+newEmployee.getJobCategory()+", Title="+newEmployee.getTitle()+", Salary="+newEmployee.getSalary()+", Contract End Year="+newEmployee.getEndYear());
		}
	}
	*/
	
	public static void manageContract(NFLEmployee newEmployee) {
		
		if(newEmployee.getEndYear() <= 2024) {
			System.out.println("This employee's contract has expired. Their contract record should be remvoed from the database.");
			System.out.println("Expired Employee Contract Info:");
			System.out.println("Name="+newEmployee.getName()+", Team="+newEmployee.getTeam()+", Job Category="+newEmployee.getJobCategory()+", Title="+newEmployee.getTitle()+", Salary="+newEmployee.getSalary()+", Contract End Year="+newEmployee.getEndYear());
		}
		else if(newEmployee.getEndYear() == 2025) {
			Scanner newScanner = new Scanner(System.in);
			System.out.println("This employee's contract needs to be updated. Please provide the updated information: ");
			System.out.println("What is their updated job title? ");
			String newJobTitle = newScanner.nextLine();
			System.out.println("What is their updated annual salary?");
			int newSalary = newScanner.nextInt();
			System.out.println("What year does their new contract end?");
			int newEndYear = newScanner.nextInt();
			
			newEmployee.newContract(newJobTitle, newSalary, newEndYear);
			System.out.println("Updated Employee Contract Info:");
			System.out.println("Name="+newEmployee.getName()+", Team="+newEmployee.getTeam()+", Job Category="+newEmployee.getJobCategory()+", Title="+newEmployee.getTitle()+", Salary="+newEmployee.getSalary()+", Contract End Year="+newEmployee.getEndYear());
		}
		else {
			System.out.println("Employee Contract Info:");
			System.out.println("Name="+newEmployee.getName()+", Team="+newEmployee.getTeam()+", Job Category="+newEmployee.getJobCategory()+", Title="+newEmployee.getTitle()+", Salary="+newEmployee.getSalary()+", Contract End Year="+newEmployee.getEndYear());
		}
	}
	/**
	 * Method: Managing the pay stub for the specific employee
	 * @param employee
	 */
	/*
	public static void managePayStub(NFLEmployee newEmployee) {
		System.out.println("Employee Monthly Pay Stub:");
		newEmployee.printPayStub();
	}
	
	public static void printContract(NFLEmployee newEmployee) {
		System.out.println("Name="+newEmployee.getName()+", Team="+newEmployee.getTeam()+", Job Category="+newEmployee.getJobCategory()+", Title="+newEmployee.getTitle()+", Salary="+newEmployee.getSalary()+", Contract End Year="+newEmployee.getEndYear());
		//System.out.println("Name="+this.getName()+", Team="+newEmployee.getTeam()+", Job Category="+newEmployee.getJobCategory()+", Title="+newEmployee.getTitle()+", Salary="+newEmployee.getSalary()+", Contract End Year="+newEmployee.getEndYear());

	}
	*/
	public static void managePayStub(String fileName) {
		//REad in the NFLPersonnel file
		try {
			File inFile = new File(fileName);
			/*
			Scanner fileScanner = new Scanner(inFile);
			// I think we have to set delimiter?
			fileScanner.useDelimiter(",");
			fileScanner.nextLine();
			*/
			FileReader fileReader = new FileReader(inFile);
			BufferedReader bufferReader = new BufferedReader(fileReader);
			String inputStr = bufferReader.readLine(new inputStr);

			
			while(inputStr != null ) {
				//Create an object for each record read in
				
				String lineStrings []= inputStr.split(",");
				
				String playerName = lineStrings[0];
				String teamName = lineStrings[1];
				String category = lineStrings[2];
				String title = lineStrings[3];
				int salary = Integer.parseInt(lineStrings[4]);
				int expirationYear = Integer.parseInt(lineStrings[5]);
				
				//Create the object for it
				NFLEmployee employee = new NFLEmployee(playerName,teamName,category,title,salary,expirationYear);
	
				//Call the printPayStub method to print the pay stub for each employee
				System.out.println("Employee Monthly Pay Stub:");
				employee.printPayStub();
			}
			} catch (FileNotFoundException e) {
				e.printStackTrace();
			}
		

		
	}
	
	public static void printContract(NFLEmployee newEmployee) {
		System.out.println("Name="+newEmployee.getName()+", Team="+newEmployee.getTeam()+", Job Category="+newEmployee.getJobCategory()+", Title="+newEmployee.getTitle()+", Salary="+newEmployee.getSalary()+", Contract End Year="+newEmployee.getEndYear());
		//System.out.println("Name="+this.getName()+", Team="+newEmployee.getTeam()+", Job Category="+newEmployee.getJobCategory()+", Title="+newEmployee.getTitle()+", Salary="+newEmployee.getSalary()+", Contract End Year="+newEmployee.getEndYear());

	}

}
